...
# ══════════════════════════════════════════════
OB = {
    "impulse_candles":  2,   # Consecutive same-direction candles for impulse
    "bos_lookback":     3,   # Swing lookback for BOS confirmation
    "min_retest_gap":   2,   # Minimum bars between OB formation and retest
...
# ══════════════════════════════════════════════
SIGNAL = {
    "wick_ratio":     1.0,   # Lower wick / body ratio for hammer confirmation
...
RISK = {
    ...
    "max_open_trades":   2,     # Max concurrent open positions
...
FILTERS = {
    ...
    "atr_period":         14,
    "min_atr_multiplier": 0.2,   # Signal ATR must be ≥ X × mean ATR
    # Trading sessions (UTC hours, inclusive) — set both to None to disable
    "session_start_utc":  None,  # 24/7 trading (disabled)
    "session_end_utc":    None,  # 24/7 trading (disabled)
